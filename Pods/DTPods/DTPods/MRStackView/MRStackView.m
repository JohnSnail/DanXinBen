...
- (void)resetPages
{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        for (NSInteger i = start; i < stop; i++) {
            ...
            MRStackViewCell *page = self.pages[i];
            
            if (self.selectedPageIndex == i) {
                page.contentView.frame = CGRectMake(15, 15, CGRectGetWidth(page.frame) - 30, CGRectGetHeight(page.frame) - 15);
            } else {
...
- (void)showPagesFullScreen:(NSInteger)index
{
    ...
    MRStackViewCell *page = (MRStackViewCell *)self.pages[index];
    if (self.delegate) {
    ...
    NSLog(@"y:%f, contentY:%f", page.frame.origin.y, self.scollView.contentOffset.y);
    
    [UIView animateWithDuration:0.4 animations:^{
        CGRect rect = page.frame;
        rect.origin.y = self.scollView.contentOffset.y - rect.origin.y;
        rect.origin.x = 0;
        rect.size.height = 400;
        page.contentView.frame = rect;
    } completion:^(BOOL finished) {
        if (finished) {
            [self.scollView addSubview:viewControllerView];
        }
    }];
}
...
{
    MRStackViewCell *page = [self.reusablePages lastObject];
