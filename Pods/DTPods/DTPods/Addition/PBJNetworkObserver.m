...
- (id)init
{
    ...
    if (self) {
        ...
        self.networkStatus = PBJNetworkStatusNoNetwork;
    }
    return self;
}
...
- (void)_networkReachability:(SCNetworkReachabilityRef)networkReachability updatedWithFlags:(SCNetworkReachabilityFlags)flags
{
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (changed || !_flags.networkObserversNotified) {
            ...
            switch (_flags.networkReachable) {
                case 1:{
                    switch (_flags.cellularConnection) {
                        case 1:{
                            self.networkStatus = PBJNetworkStatus3G;
                        }
                            break;
                        case 0:{
                            self.networkStatus = PBJNetworkStatusWifi;
                        }
                ...
                case 0:
                    switch (_flags.cellularConnection) {
                        case 1:{
                            self.networkStatus = PBJNetworkStatusNoNetwork3G;
                        }
                            break;
